...
export default function AdminScreen() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        // a OMIE costuma mandar clientes em "clientes_cadastro"
        const lista = data.clientes_cadastro || data || [];
        setClients(lista);
      } catch (e) {
        console.log("Erro ao buscar clientes OMIE:", e);
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Clientes da OMIE</Text>
      <FlatList
        data={clients}
        ...
        renderItem={({ item }) => (
          <View style={styles.card}>
            <Text style={styles.name}>{item.razao_social || "Sem razão social"}</Text>
