...
router.post("/cron/sync-omie-mf-movements", requireAdmin, async (_req, res) => {
    try {
        ...
        const { data: maxRow, error: maxErr } = await supabase_1.supabaseAdmin
        ...
        const lastUpdatedAt = maxRow?.updated_at ? new Date(maxRow.updated_at) : new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);
        ...
        for (let i = 0; i < items.length; i += BATCH) {
            const batch = items.slice(i, i + BATCH).map((m) => {
            ...
            }).filter((x) => x.cod_mov_cc && Number.isFinite(x.cod_mov_cc));
            if (!batch.length)
                continue;
...
async function fetchOmieMfMovementsUpdatedSince(_since) {
